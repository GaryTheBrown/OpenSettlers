...
namespace Extractor{
	namespace Settlers3{
		MAPSettlers::MAPSettlers(Functions::DataReader* reader,unsigned int offset,unsigned int size,unsigned int cryptKey):MAPParts(reader,offset,size,cryptKey,false){
		...
		MAPSettlers::~MAPSettlers(){
			delete [] this->settlers;
		}

		std::string MAPSettlers::HeaderToString(){
...
*/
		...
		void MAPSettlers::SaveFileData(std::string location){
